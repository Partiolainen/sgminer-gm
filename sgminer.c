...
static void *stratum_sthread(void *userdata)
{
  ...
  while (42) {
    ...
    if(pool->algorithm.type == ALGO_ETHASH) {
      ...
      sshare->id = swork_id++;
      mutex_unlock(&sshare_lock);
      snprintf(ss, s_size, "{\"id\": %d, \"method\": \"mining.submit\", \"params\": [\"%s\", \"%s\", \"0x%s\", \"0x%s\", \"0x%s\"]}", sshare->id, pool->rpc_user, work->job_id, ASCIINonce, ASCIIPoWHash, ASCIIMixHash);
      ...
      snprintf(ss, s_size, "{\"method\": \"submit\", \"params\": {\"id\": \"%s\", \"job_id\": \"%s\", \"nonce\": \"%s\", \"result\": \"%s\"}, \"id\":%d}", pool->XMRAuthID, work->job_id, ASCIINonce, ASCIIResult, sshare->id);
      ...
      sshare->id = swork_id++;
      mutex_unlock(&sshare_lock);
      snprintf(ss, s_size, "{\"id\": %d, \"method\": \"mining.submit\", \"params\": [\"%s\", \"%s\", \"%s\", \"%s\", \"%s\"]}", sshare->id, pool->rpc_user, work->job_id, work->ntime, nonce+strlen(work->nonce1), solution);
...
static void gen_stratum_work(struct pool *pool, struct work *work)
{
  ...
  cg_dwlock(&pool->data_lock);
	...
	if (pool->algorithm.type == ALGO_SIA) {
	  unsigned char *cbbuf = (unsigned char *) alloca(1 + pool->swork.cb_len);
	  cbbuf[0] = 0;
  ...
  else if (pool->algorithm.type != ALGO_DECRED && pool->algorithm.type != ALGO_PASCAL) {  
